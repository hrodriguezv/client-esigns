...
import static org.icepdf.ri.util.PropertiesManager.PROPERTY_DEFAULT_FILE_PATH;
import static org.icepdf.ri.util.PropertiesManager.PROPERTY_DEFAULT_URL;
...
public class WindowManager implements WindowManagementCallback {
	...
	protected static int newWindowInvocationCounter = 0;
	...
	protected Controller commonWindowCreation() {
		...
		SwingViewBuilder factory = new SwingViewBuilder((SwingController) controller, viewType, pageFit,
				pageRotation);
	...
	/**
	 * Loads the last used windows location as well as other frame related settings
	 * and insures the frame is visible.
	 *
	 * @param frame
	 *            parent window containers.
	 */
	...
	public static void newWindowLocation(Container frame, Preferences prefs) {
		...
		// quick check to make sure the viewer will be visible in at least one screen,
		// if not we default to primary
		...
		// no intersection with a current screen then we go with the centering
		// coordinates of the primary screen.
		if (results.size() == 0) {
			previousX = x;
			previousY = y;
		}
	...
	/**
	 * As long as no windows have opened or closed, then the indexes in the returned
	 * list should still be valid for doing operations on the respective Controller
	 * objects
	 *
	 * @param giveIndex
	 *            Give this SwingControllers index in the list as an Integer
	 *            appended to the List
	 * @return List of String objects, each representing an open Document's origin.
	 *         The last element may be an Integer
	 */
