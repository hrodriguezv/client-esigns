...
public class BgpButtonUI extends BasicButtonUI {
  ...
  protected final void initShape(Component c) {
    ...
    if (!c.getBounds().equals(base)) {
      ...
      shape = new RoundRectangle2D.Double(0, 0, c.getWidth() - 1, c.getHeight() - 1, ARC_WIDTH,
          ARC_HEIGHT);

      border = new RoundRectangle2D.Double(FOCUS_STROKE, FOCUS_STROKE,
          c.getWidth() - 1 - FOCUS_STROKE * 2, c.getHeight() - 1 - FOCUS_STROKE * 2, ARC_WIDTH,
          ARC_HEIGHT);
  ...
  protected final void paintFocusAndRollover(Graphics2D g2, Component c, Color color) {
    ...
    g2.setPaint(
      new GradientPaint(0, 0, color, c.getWidth() - 1, c.getHeight() - 1, color.brighter(), true));
