...
/**
 * <p>
 * Launches the Viewer Application. The following parameters can be used to
 * optionally load a PDF document at startup.
 * </p>
 ...
 * <td>Starts the ICEpdf Viewer and displays the specified local PDF file. Use
 * the following syntax: <br>
 * -loadfile c:/examplepath/file.pdf</td>
 * </tr>
 * <tr>
 * <td>-loadfile <i>filename</i></td>
 * <td>Starts the ICEpdf Viewer and displays the PDF file at the specified URL.
...
public class Launcher {
	...
	public static void main(String[] argv) {
		...
		// start the viewer
		run(contentFile, contentURL, sessionId, messageBundle);
	}
	 ...
	 *
	 * @param contentFile
	 *            URI of a file which will be loaded at runtime, can be null.
	 * @param contentURL
	 *            URL of a file which will be loaded at runtime, can be null.
	 * @param messageBundle
	 *            messageBundle to pull strings from
	 */
	private static void run(String contentFile, String contentURL, String sessionId, ResourceBundle messageBundle) {

		// initiate the properties manager.
		propertiesManager = PropertiesManager.getInstance();

		// initiate font Cache manager, reads system if necessary, load the cache
		// otherwise.
		...
		ViewModel.setDefaultFilePath(
				propertiesManager.getPreferences().get(PropertiesManager.PROPERTY_DEFAULT_FILE_PATH, null));
		ViewModel.setDefaultURL(propertiesManager.getPreferences().get(PropertiesManager.PROPERTY_DEFAULT_URL, null));

		// application instance
		WindowManager windowManager = null;
		if ((sessionId != null) && (!sessionId.isEmpty())) {
			windowManager = WindowExtendedManager.createInstance(propertiesManager, messageBundle, sessionId);
			contentFile = FileSystemManager.getInstance().getPdfDocument().getAbsolutePath();
		} else {
			windowManager = WindowExtendedManager.createInstance(propertiesManager, messageBundle);
		}
		...
		if (((contentFile == null || contentFile.length() == 0) && (contentURL == null || contentURL.length() == 0))
	...
	/**
	 * If a L&F has been specifically set then try and use it. If not then resort to
	 * the 'native' system L&F.
	 *
	 * @param messageBundle
	...
	private static void setupLookAndFeel(ResourceBundle messageBundle) {
		...
		String className = propertiesManager.getLookAndFeel(APPLICATION_LOOK_AND_FEEL, null, messageBundle);
		...
		if (className != null) {
			try {
				...
				// setup a patterned message
				Object[] messageArguments = { preferences.get(APPLICATION_LOOK_AND_FEEL, null) };
				MessageFormat formatter = new MessageFormat(
						messageBundle.getString("viewer.launcher.URLError.dialog.message"));
